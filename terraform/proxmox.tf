...
resource "proxmox_virtual_environment_download_file" "almalinux_9_latest_cloud_image" {
  ...
  checksum           = "0a07f59b59efba1bbdbe07e69c11e291f015e602ed614596f064ebe00c1bdf9b"
...
module "proxmox_cloud_init_config_hermes01" {
  source                                                  = "./modules/proxmox_cloud_init_config"
  proxmox_cloud_init_config_vm_name                       = "hermes01"
  proxmox_cloud_init_config_virtual_environment_node_name = "proxmox01"
  proxmox_cloud_init_config_ssh_authorized_keys           = data.onepassword_item.ssh_key.public_key
}

resource "proxmox_virtual_environment_vm" "hermes01" {
  name        = "hermes01"
  vm_id       = 105
  description = "Created with OpenTofu."
  tags        = ["vm", "opentofu", "tailscale", "192.168.0.5"]
  node_name   = "proxmox01"
  on_boot     = false
  started     = false
  ...
  initialization {
    ...
    user_data_file_id = module.proxmox_cloud_init_config_hermes01.cloud_init_config_id
  }
