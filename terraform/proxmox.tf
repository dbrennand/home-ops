...
module "proxmox_cloud_init_config_apps01" {
  source                                                  = "./modules/proxmox_cloud_init_config"
  proxmox_cloud_init_config_vm_name                       = "apps01"
  proxmox_cloud_init_config_virtual_environment_node_name = "proxmox01"
  proxmox_cloud_init_config_ssh_authorized_keys           = data.onepassword_item.ssh_key.public_key
}

resource "proxmox_virtual_environment_vm" "apps01" {
  name        = "apps01"
  vm_id       = 107
  description = "Created with OpenTofu."
  tags        = ["vm", "opentofu", "tailscale", "192.168.0.12"]
  node_name   = "proxmox01"
  on_boot     = false
  started     = true
  ...
  initialization {
    ...
    user_data_file_id = module.proxmox_cloud_init_config_apps01.cloud_init_config_id
  }
