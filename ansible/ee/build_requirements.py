...
PYPROJECT_TOML = EE_DIR.parent.parent / "pyproject.toml"
...
def build_collection_package_map() -> dict[str, list[str]]:
    ...
    # ansible-builder reads both locations for collection Python requirements
    candidate_globs = ["*/*/requirements.txt", "*/*/meta/ee-requirements.txt"]

    for glob_pattern in candidate_globs:
        for req_file in sorted(COLLECTIONS_DIR.glob(glob_pattern)):
            rel = req_file.relative_to(COLLECTIONS_DIR)
            # Extract namespace/name from path regardless of depth
            namespace, name = rel.parts[0], rel.parts[1]
...
def resolve_with_uv_pip_compile() -> str:
    """Run uv pip compile on collection-requirements.txt + pyproject.toml; return stdout."""
...
def render_requirements(compiled: str, pkg_map: dict[str, list[str]]) -> str:
    ...
    - Direct deps from collection-requirements.txt are annotated with the
      collection name(s) that declared them, e.g. "awx.awx (collection)".
    - Direct deps from pyproject.toml (ansible-core and its transitive deps)
      are annotated as "pyproject.toml".
    """
    header = (
    ...
    while i < len(lines):
        ...
        if not line[0].isspace():   # package specifier line
            ...
            coll_direct = [r for r in refs if "collection-requirements" in r]
            pyproject_direct = [r for r in refs if "pyproject.toml" in r]
            transitive = [r for r in refs if r not in coll_direct and r not in pyproject_direct]
            ...
            if coll_direct:
                ...
                if colls:
                    ...
                    new_refs.extend(coll_direct)  # fallback: keep original
            if pyproject_direct:
                new_refs.append("pyproject.toml")
